...
- Active Java execution is process-local. Durable task checkpoints support investigation and lease recovery, but automatic restart-and-resume of every in-flight asynchronous scenario is not implemented.
- The deterministic assessment executor creates reproducible lifecycle evidence. The lower-level `LifecycleExecution` performs real isolated patch, Maven validation, repair, and rollback; connecting every deterministic catalog scenario to generated source mutation remains a future consolidation step.
